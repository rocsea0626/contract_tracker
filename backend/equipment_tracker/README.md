...
```bash
contract_tracker$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
contract_tracker$ sam local start-api
contract_tracker$ curl http://localhost:3000/
```
...
```bash
contract_tracker$ sam logs -n HelloWorldFunction --stack-name contract_tracker --tail
```
...
```bash
aws cloudformation delete-stack --stack-name contract_tracker
```
